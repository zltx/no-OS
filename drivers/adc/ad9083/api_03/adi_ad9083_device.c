...
int32_t adi_ad9083_device_init(adi_ad9083_device_t *device)
{
    ...
    err = adi_ad9083_hal_log_write(device, ADI_CMS_LOG_MSG, "api v%d.%d.%d commit %s for ad%x r1",
    ad9083_api_revision[0], ad9083_api_revision[1], ad9083_api_revision[2], "5cf62d0", AD9083_ID);
    ...
    err = adi_ad9083_hal_log_write(device, ADI_CMS_LOG_MSG, "sizeof(uint8/16/32/64_t) = %d/%d/%d/%d bytes",
...
int32_t adi_ad9083_device_spi_config(adi_ad9083_device_t *device)
{
    ...
    return adi_ad9083_hal_reg_set(device, REG_SPI_INTERFACE_CONFIG_A_ADDR, reg_val);
}
...
int32_t adi_ad9083_device_reg8_access_check(adi_ad9083_device_t *device)
{
    ...
    AD9083_ERROR_RETURN(err);
    if (data8 != 0x5a) {
        err = adi_ad9083_hal_log_write(device, ADI_CMS_LOG_ERR, "8bit r/w test failed. Write %.2x but readback is %.2x.", 0x5a, data8);
        AD9083_ERROR_RETURN(err);
    ...
    AD9083_ERROR_RETURN(err);
    if (data8 != 0xa5) {
        err = adi_ad9083_hal_log_write(device, ADI_CMS_LOG_ERR, "8bit r/w test failed. Write %.2x but readback is %.2x.", 0xa5, data8);
        AD9083_ERROR_RETURN(err);
        return API_CMS_ERROR_TEST_FAILED;
    }
...
int32_t adi_ad9083_device_reset(adi_ad9083_device_t *device, adi_ad9083_reset_e operation)
{
    ...
    /* do reset */
    if ((operation == AD9083_SOFT_RESET) || (operation == AD9083_SOFT_RESET_AND_INIT)) {
        err = adi_ad9083_hal_reg_set(device, REG_SPI_INTERFACE_CONFIG_A_ADDR, 0x81);
        AD9083_ERROR_RETURN(err);
        err = adi_ad9083_hal_delay_us(device, 200);
        AD9083_ERROR_RETURN(err);
    } else if ((operation == AD9083_HARD_RESET) || (operation == AD9083_HARD_RESET_AND_INIT)) {
    ...
    /* do init */
    if ((operation == AD9083_SOFT_RESET_AND_INIT) || (operation == AD9083_HARD_RESET_AND_INIT)) {
...
int32_t adi_ad9083_device_clock_config_set(adi_ad9083_device_t *device, uint64_t adc_clk_hz, uint64_t ref_clk_hz)
{
    ...
    for (ref_div = 1; ref_div < 6; ref_div++) {
        ...
        for (m_div = 2; m_div < 52; m_div++) {
            for (n_div = 0; n_div < 4; n_div++) {
                ...
                #endif
                    if ((vco_freq > 8800000000ul) &&
                        (vco_freq < 12400000000ul) &&
                        (adc_sample_rate == adc_clk_hz) &&
                        (pfd_clk_hz <= 500000000ul)) {
                        valid_cfg = 1;
                        break;
                    }
                }
    ...
    if ((vco_freq < 8800000000ul) || (vco_freq > 12400000000ul) || (pfd_clk_hz < 25000000ul) || (pfd_clk_hz > 500000000ul)) {
    ...
    if (pll_lock != 0x1) {
        AD9083_ERROR_REPORT(API_CMS_ERROR_PLL_NOT_LOCKED, pll_lock, "PLL not locked");
...
int32_t adi_ad9083_device_power_status_check(adi_ad9083_device_t *device)
{
    ...
    AD9083_ERROR_RETURN(err);
    power_on &= reg8;
    err = adi_ad9083_hal_bf_get(device, BF_PORB_VDDSYNTH_LCPLL_1P0_INFO, &reg8, 1);
    AD9083_ERROR_RETURN(err);
    power_on &= reg8;

    if (power_on == 0) {
        err = adi_ad9083_hal_log_write(device, ADI_CMS_LOG_ERR, "some power supplies are not on.");
        AD9083_ERROR_RETURN(err);
    }
...
int32_t adi_ad9083_device_temp_sensor_enable_set(adi_ad9083_device_t *device, uint8_t enable)
{
    ...
    err = adi_ad9083_hal_bf_set(device, BF_EN_TEMP_DIODE_INFO, (enable > 0) ? 0x03 : 0);
    AD9083_ERROR_RETURN(err);
    return API_CMS_ERROR_OK;
}
...
int32_t adi_ad9083_device_temp_get(adi_ad9083_device_t *device, int32_t vbe_1, int32_t vbe_2, int32_t *temp)
{
    ...
    int32_t constant = 3874;                               /* q * 1000 / (k * ln20): q = 1.602e-19, k = 1.3806e-23, ln20 = 2.9957 */
    int32_t temp_mC = constant * (vbe_1 - vbe_2) - 273150; /* unit: mili-degree, vbe_1 & vbe_2 must be in unit mV */
    *temp = temp_mC;

    return API_CMS_ERROR_OK;
}

int32_t adi_ad9083_device_low_power_pin_ctrl_set(adi_ad9083_device_t *device, uint8_t sel)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_device_low_power_cfg_set(adi_ad9083_device_t *device, uint8_t cfg)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_device_low_power_status_get(adi_ad9083_device_t *device, uint8_t *status)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_device_low_power_pol_set(adi_ad9083_device_t *device, uint8_t pol)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_device_low_power_mask_set(adi_ad9083_device_t *device, uint8_t mask)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
