...
int32_t adi_ad9083_hal_reset_pin_ctrl(adi_ad9083_device_t *device, uint8_t enable)
{
    AD9083_NULL_POINTER_RETURN(device);
...
int32_t adi_ad9083_hal_log_write(adi_ad9083_device_t *device, adi_cms_log_type_e log_type,
    const char* comment, ...)
{
    int32_t err;
    va_list argp;
    ...
    err = device->hal_info.log_write(device->hal_info.user_data, log_type, comment, argp);
    va_end(argp);

    return err;
}

int32_t adi_ad9083_hal_bf_get(adi_ad9083_device_t *device, uint32_t reg, uint32_t info, uint8_t *value, uint8_t value_size_bytes)
{
    int32_t  err;
...
int32_t adi_ad9083_hal_bf_set(adi_ad9083_device_t *device, uint32_t reg, uint32_t info, uint64_t value)
{
    int32_t  err;
    uint8_t  reg_offset = 0, data8 = 0;
    uint8_t  offset = (uint8_t)(info >> 0), width  = (uint8_t)(info >> 8);
...
int32_t adi_ad9083_hal_reg_get(adi_ad9083_device_t *device , uint32_t reg, uint8_t *data)
{
    ...
    if (reg < 0x1000) {
        ...
        in_data[1] = (reg & 0xFF);
        if (API_CMS_ERROR_OK != device->hal_info.spi_xfer(device->hal_info.user_data,
            in_data, out_data, SPI_IN_OUT_BUFF_SZ)) {
...
int32_t adi_ad9083_hal_reg_set(adi_ad9083_device_t *device, uint32_t reg, uint8_t data)
{
    uint8_t in_data[SPI_IN_OUT_BUFF_SZ] = {0};
    uint8_t out_data[SPI_IN_OUT_BUFF_SZ] = {0};
    AD9083_NULL_POINTER_RETURN(device);
...
int32_t adi_ad9083_hal_error_report(adi_ad9083_device_t* device, adi_cms_log_type_e log_type,
    ...
    const char* var_name, const char* comment)
{
    ...
    if (API_CMS_ERROR_OK != adi_ad9083_hal_log_write(device, log_type, "%s, \"%s\" in %s(...), line%d in %s",
...
int32_t adi_ad9083_hal_cbuspll_reg_get(adi_ad9083_device_t *device, uint32_t reg, uint8_t* data)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_NULL_POINTER_RETURN(data);
...
int32_t adi_ad9083_hal_cbuspll_reg_set(adi_ad9083_device_t *device, uint32_t reg, uint8_t data)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
...
int32_t adi_ad9083_hal_div_nume_deno(adi_ad9083_device_t *device, uint64_t a, uint64_t b, uint8_t *integer, uint8_t *nume, uint8_t *deno)
{
...
void adi_ad9083_hal_add_128(uint64_t ah, uint64_t al, uint64_t bh, uint64_t bl, uint64_t *hi, uint64_t *lo)
{
...
void adi_ad9083_hal_subt_128(uint64_t ah, uint64_t al, uint64_t bh, uint64_t bl, uint64_t *hi, uint64_t *lo)
{
    uint64_t rl, rh;
    if (bl <= al) {
        rl = al - bl;
...
void adi_ad9083_hal_mult_128(uint64_t a, uint64_t b, uint64_t *hi, uint64_t *lo)
{
...
void adi_ad9083_hal_div_128(uint64_t a_hi, uint64_t a_lo, uint64_t b_hi, uint64_t b_lo, uint64_t *hi, uint64_t *lo)
{
    ...
    do {
        if ((remain_hi > part1_hi) || ((remain_hi == part1_hi) && (remain_lo >= part1_lo))) {
            adi_ad9083_hal_subt_128(remain_hi, remain_lo, part1_hi, part1_lo, &remain_hi, &remain_lo);
            adi_ad9083_hal_add_128(result_hi, result_lo, mask_hi, mask_lo, &result_hi, &result_lo);
        }
