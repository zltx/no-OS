...
#define AD9083_MSG_REPORT(var, comment) \
    adi_ad9083_hal_error_report(device, ADI_CMS_LOG_MSG,  API_CMS_ERROR_OK, __FILE__, __FUNCTION_NAME__, __LINE__, #var, comment)
#define AD9083_WARN_REPORT(var, comment) \
    adi_ad9083_hal_error_report(device, ADI_CMS_LOG_WARN, API_CMS_ERROR_OK, __FILE__, __FUNCTION_NAME__, __LINE__, #var, comment)
#define AD9083_ERROR_REPORT(error, var, comment) \
    adi_ad9083_hal_error_report(device, ADI_CMS_LOG_ERR, error, __FILE__, __FUNCTION_NAME__, __LINE__, #var, comment)

/* log report */
#define AD9083_LOG_FUNC() \
    adi_ad9083_hal_log_write(device, ADI_CMS_LOG_MSG, "%s(...)", __FUNCTION_NAME__)
...
{ \
    if(p == NULL) { \
        AD9083_ERROR_REPORT(API_CMS_ERROR_NULL_PARAM, p, "Null pointer passed."); \
...
{ \
    if(r) { \
        AD9083_ERROR_REPORT(API_CMS_ERROR_INVALID_PARAM, r, "Invalid param passed."); \
