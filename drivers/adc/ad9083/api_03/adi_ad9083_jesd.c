...
/*============= C O D E ====================*/
int32_t adi_ad9083_jesd_powerup_enable_set(adi_ad9083_device_t *device, uint8_t enable)
{
    int32_t err;
...
int32_t adi_ad9083_jesd_tx_link_reset(adi_ad9083_device_t *device, uint8_t reset)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_jesd_tx_link_enable_set(adi_ad9083_device_t *device, uint8_t reset)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_jesd_tx_link_config_set(adi_ad9083_device_t *device, adi_cms_jesd_param_t *jesd_param)
{
    ...
    err = adi_ad9083_hal_bf_set(device, BF_JTX_M_CFG_INFO, jesd_param->jesd_m - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_F_CFG_INFO, jesd_param->jesd_f - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_K_CFG_INFO, jesd_param->jesd_k - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_CS_CFG_INFO, jesd_param->jesd_cs);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_N_CFG_INFO, jesd_param->jesd_n - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_NP_CFG_INFO, jesd_param->jesd_np - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_SUBCLASSV_CFG_INFO, jesd_param->jesd_subclass);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_S_CFG_INFO, jesd_param->jesd_s - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_NS_CFG_INFO, 0);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_L_CFG_INFO, jesd_param->jesd_l - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_SCR_CFG_INFO, jesd_param->jesd_scr);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_JESDV_CFG_INFO, 1);                       /* always JESD204B */
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_HD_CFG_INFO, jesd_param->jesd_hd);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_DID_CFG_INFO, jesd_param->jesd_did);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_BID_CFG_INFO, jesd_param->jesd_bid);
    AD9083_ERROR_RETURN(err);
    ...
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_MODE_S_SEL_INFO, 0);                      /* disable quick config */
    AD9083_ERROR_RETURN(err);
    return API_CMS_ERROR_OK;
}

int32_t adi_ad9083_jesd_tx_lan_xbar_set(adi_ad9083_device_t *device, uint8_t physical_lane, uint8_t logical_lane)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();

    err = adi_ad9083_hal_bf_set(device, BF_JTX_LANE_SEL_INFO(physical_lane), logical_lane);
    AD9083_ERROR_RETURN(err);
    return API_CMS_ERROR_OK;
}

int32_t adi_ad9083_jesd_tx_lan_power_down_set(adi_ad9083_device_t *device, uint8_t physical_lane, uint8_t enable)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();

    err = adi_ad9083_hal_bf_set(device, BF_JTX_FORCE_LANE_PD_INFO(physical_lane), enable);
    AD9083_ERROR_RETURN(err);
    return API_CMS_ERROR_OK;
}
...
int32_t adi_ad9083_jesd_tx_clk_set(adi_ad9083_device_t *device)
{
    ...
    uint8_t ifx_pclk_div_integer , ifx_pclk_div_numerator , ifx_pclk_div_denominator;
    ...
    while (((vco_clk < 8000000000ul) || (vco_clk > 16000000000ul)) && (bit_repeat < 33)) {
...
#endif
    err = adi_ad9083_hal_div_nume_deno(device, lane_rate * 2, adc_clk, &spi_div_integer, &spi_div_numerator, &spi_div_denominator);
    AD9083_ERROR_RETURN(err);
...
#endif
    err = adi_ad9083_hal_div_nume_deno(device, vco_clk, fPfd * 8, &b_lcpll_integer, &b_lcpll_numerator, &b_lcpll_denominator);
    AD9083_ERROR_RETURN(err);

    while (((fPfd < 40000000ul) || (fPfd > 400000000ul) || (b_lcpll_numerator > 0)) && (refindiv_lcpll < 15)) {
    ...
    clk_divp = vco_clk >> 3;
    err = adi_ad9083_hal_div_nume_deno(device, adc_clk / 2, lane_rate / 40, &pclk_div_integer, &pclk_div_numerator, &pclk_div_denominator);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_div_nume_deno(device, adc_clk / 2, (lane_rate * bit_repeat) / 40, &ifx_pclk_div_integer, &ifx_pclk_div_numerator, &ifx_pclk_div_denominator);
    AD9083_ERROR_RETURN(err);
#ifdef __KERNEL__
    ...
    err = adi_ad9083_hal_bf_set(device, BF_JTX_BR_LOG2_RATIO_INFO(0), bit_repeat_exponent); /* lane 1 */
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_BR_LOG2_RATIO_INFO(1), bit_repeat_exponent); /* lane 2 */
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_BR_LOG2_RATIO_INFO(2), bit_repeat_exponent); /* lane 3 */
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_BR_LOG2_RATIO_INFO(3), bit_repeat_exponent); /* lane 4 */
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_SPI_DIV_SERDES_INFO, spi_div);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_DIVP_LCPLL_RC_INFO, 0); /* multiplication factor is 8 for B divider */
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_REFINDIV_LCPLL_RC_INFO, lcpll_exponent);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_B_LCPLL_RC_INFO, b_lcpll_integer);
    AD9083_ERROR_RETURN(err);

    err = adi_ad9083_hal_bf_set(device, BF_JTX_PCLK_DIV_INTEGER_INFO, pclk_div_integer - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_PCLK_DIV_FRAC_NUM_INFO, pclk_div_numerator);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_PCLK_DIV_FRAC_DEN_INFO, pclk_div_denominator);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_IFX_PCLK_DIV_INTEGER_INFO, ifx_pclk_div_integer - 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_IFX_PCLK_DIV_FRAC_NUM_INFO, ifx_pclk_div_numerator);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_JTX_IFX_PCLK_DIV_FRAC_DEN_INFO, ifx_pclk_div_denominator);
    AD9083_ERROR_RETURN(err);

    err = adi_ad9083_hal_bf_set(device, BF_DIVM_LCPLL_RC_INFO, 1);
    AD9083_ERROR_RETURN(err);
    err = adi_ad9083_hal_bf_set(device, BF_DIVM_LCPLL_RC_RX_INFO, divm_lcpll_rc_rx);
    AD9083_ERROR_RETURN(err);
...
int32_t adi_ad9083_jesd_tx_gen_test(adi_ad9083_device_t *device, adi_ad9083_jesd_tx_test_data_src_e data_source, adi_ad9083_jesd_tx_test_mode_e test_mode)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_LOG_FUNC();
...
int32_t adi_ad9083_jesd_tx_continuous_d215_enable_set(adi_ad9083_device_t *device, uint8_t lane_id, uint8_t enable)
{
    ...
    err = adi_ad9083_hal_bf_set(device, BF_JTX_DL_204B_SCR_DATA_SEL_CFG_INFO(lane_id), enable);
    AD9083_ERROR_RETURN(err);
...
//    }
    ...
    err = adi_ad9083_jesd_tx_gen_test(device, AD9083_JESD_TX_TEST_DATA_PHY, pattern);
    AD9083_ERROR_RETURN(err);
...
int32_t adi_ad9083_jesd_tx_check_jtx_m(adi_ad9083_device_t *device, uint8_t m)
{
    ...
    if (m != total_channels)
    {
        err = adi_ad9083_hal_log_write(device, ADI_CMS_LOG_WARN, "jtx_m NOT match non-zero NCO number.");
        AD9083_ERROR_RETURN(err);
    }
...
int32_t adi_ad9083_jesd_tx_lcpll_status_get(adi_ad9083_device_t *device, uint8_t *pll_locked)
{
    ...
    err = adi_ad9083_hal_bf_get(device, BF_LCPLLLOCK_LCPLL_RS_INFO, pll_locked, 1);
    AD9083_ERROR_RETURN(err);
...
int32_t adi_ad9083_jesd_tx_link_status_get(adi_ad9083_device_t *device, uint16_t *status)
{
    ...
    err = adi_ad9083_hal_bf_get(device, BF_JTX_DL_204B_STATE_INFO, &state_204b, 1);
    AD9083_ERROR_RETURN(err);
    res = state_204b & 0x0F;

    err = adi_ad9083_hal_bf_get(device, BF_JTX_DL_204B_SYNC_N_INFO, &sync_204b, 1);
    AD9083_ERROR_RETURN(err);
    res += (sync_204b & 0x1) << 4;
    ...
    err = adi_ad9083_hal_bf_get(device, BF_JTX_PHASE_ESTABLISHED_INFO, &phase_establish, 1);
    AD9083_ERROR_RETURN(err);
    res += (phase_establish & 0x1) << 6;

    err = adi_ad9083_hal_bf_get(device, BF_JTX_INVALID_MODE_INFO, &invalid_mode, 1);
    AD9083_ERROR_RETURN(err);
...
int32_t adi_ad9083_jesd_tx_set_swing(adi_ad9083_device_t *device, uint8_t lane, adi_ad9083_ser_swing_e swing)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
...
int32_t adi_ad9083_jesd_tx_set_pre_emp(adi_ad9083_device_t *device, uint8_t lane, adi_ad9083_ser_pre_emp_e pre_emp)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_INVALID_PARAM_RETURN(lane >= AD9083_JESD_SER_COUNT);
...
int32_t adi_ad9083_jesd_tx_set_post_emp(adi_ad9083_device_t *device, uint8_t lane, adi_ad9083_ser_post_emp_e post_emp)
{
    int32_t err;
    AD9083_NULL_POINTER_RETURN(device);
    AD9083_INVALID_PARAM_RETURN(lane >= AD9083_JESD_SER_COUNT);
...
int32_t adi_ad9083_jead_tx_ser_settings_set(adi_ad9083_device_t *device, adi_ad9083_ser_settings_t *ser_settings)
{
    int32_t err;
    uint8_t i;
    AD9083_NULL_POINTER_RETURN(device);
    ...
    for (i = 0; i < AD9083_JESD_SER_COUNT; ++i) {
        err = adi_ad9083_jesd_tx_set_swing(device, i, ser_settings->indv_ser_lane_settings[i].swing_setting);
        AD9083_ERROR_RETURN(err);
        err = adi_ad9083_jesd_tx_set_pre_emp(device, i, ser_settings->indv_ser_lane_settings[i].pre_emp_setting);
        AD9083_ERROR_RETURN(err);
        err = adi_ad9083_jesd_tx_set_post_emp(device, i, ser_settings->indv_ser_lane_settings[i].post_emp_setting);
        AD9083_ERROR_RETURN(err);
    }

    /* tx invert */
    err = adi_ad9083_hal_reg_set(device, REG_MAIN_DATA_INV_ADDR, ser_settings->tx_invert_mask);
    AD9083_ERROR_RETURN(err);
