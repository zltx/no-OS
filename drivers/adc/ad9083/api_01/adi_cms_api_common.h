 ...
 * @param  out_data    Pointer to array where the data to which the SPI will be
 * written
 * @param  size_bytes  The size in bytes allocated for each of the in_data and
 * out_data arrays. bit[31:28]: 0000-8bit reg data, 0001-16bit reg data,
 * 0010-32bit reg data
 *
 ...
 *         Performs a blocking or sleep delay for the specified time in
 * microseconds The implementation of this function is platform dependent and is
 * required for correct operation of the API.
 *
 ...
 * any parameters/settings that may be required by the function to write log
 * messages for the ADI Device.
 ...
 *         This function shall initialize all external hardware resources
 * required by the ADI Device and API for correct functionality as per the
 *         target platform.
 *         For example initialization of SPI, GPIO resources, clocks etc.
 *
 * @param  user_data  A void pointer to a client defined structure containing
 * any parameters/settings that may be required by the function to initialize
 * the hardware for the ADI Device.
 *
 * @return 0 for success
 * @return Any non-zero value indicates an error
 */
 ...
 *         For example initialization of SPI, GPIO resources, clocks etc.
 *         It should close and free any resources assigned in the hw_open_t
 * function.
 *
 * @param  user_data  A void pointer to a client defined structure containing
 * any parameters/settings that may be required by the function to
 * close/shutdown the hardware for the ADI Device.
 *
 * @return 0 for success
 * @return Any non-zero value indicates an error
 */
 ...
 * function to control the hardware for the ADI Device TX_ENABLE PIN.
 ...
 * @param  user_data  A void pointer to a client defined structure containing
 *                    any parameters/settings that may be required by the
 * function to control the hardware for the ADI Device RESETB PIN.
 * @param  enable     A uint8_t value indicating the desired enable/disable
 ...
 *                      any parameters/settings that may be required by the
 * function to control the hardware for the ADI Device RESETB PIN.
 * @param   gpio        A uint32_t GPIO index used for identification. See enum
 * "adi_adl5580_gpio_e".
 * @param   value       A uint32_t value indicating the desired high/low state
 * for GPIO. A value of 1 indicates GPIO pin is set HIGH A value of 0 indicates
 * GPIO pin is set LOW
 *
 * @return 0 for success
 * @return Any non-zero value indicates an error
 ...
 * @param   gpio        A uint32_t GPIO index used for identification. See enum
 * "adi_adl5580_gpio_e".
 * @param   value       A uint32_t integer pointer indicating the readback state
 * of GPIO. A value of 1 indicates GPIO pin is set HIGH A value of 0 indicates
 * GPIO pin is set LOW
 * @Note    Depending on the platform, reading a GPIO which is set as an OUTPUT
 * may result in changing the GPIO state.
 * @return 0 for success
 * @return Any non-zero value indicates an error
 */
